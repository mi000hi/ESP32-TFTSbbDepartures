...
// int ntp_time_getMonth() {
//   return timeClient.getMonth();
// }

// int ntp_time_getYear() {
...
bool check_summer_time(unsigned long sec_since_epoch) {
	...
	struct tm *timeInfo = localtime(&epochSeconds);

	return timeInfo->tm_isdst;
	// // Implement DST rules for Switzerland
	// if (month < 3 || month > 10) {
	//         is_summer_time = false; // Not in DST period
	// }
	// if (month > 3 && month < 10) {
	//         is_summer_time = true; // In DST period
	// }
	// int lastSunday = 31 - (dayOfWeek(year, month, 31) - 1);
	// if ((month == 3 && day >= lastSunday) || (month == 10 && day < lastSunday)) {
	//         is_summer_time = true; // In DST period
	// }
	// is_summer_time = false; // Not in DST period
}
